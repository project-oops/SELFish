...
| **craziiEmu**<br/>`craziiEmu@8a13647` | `src/CraziiEmu.Libs/Agc/AgcExports.cs` - `sceAgcCreateShader` as an open-source PS5 emulator reads the AGC shader container: the `0x34333231` magic, the `0x18` version, the pointer table at `0x08`-`0x38`, and the shader-type and register-count bytes at `0x5A`/`0x5C` | the only citable source for the AGC shader container. Established which of the fields in obSCEne's `sceAgcCreateShader` request are derivable and which are known only from a vendor disassembly, and so must stay out. (D103) |
